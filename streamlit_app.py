# streamlit_app.py  — adds a “deal blurb” line under each card
# ════════════════════════════════════════════════════════════
...
# ────────────────── deal‑blurb extraction ────────────────────────────────────
def extract_blurb(raw: str, matched_label: str) -> str:
    """
    Return ≤ 280 characters surrounding the first regex hit that belongs
    to the detected label.  Falls back to the first <li> line that looks
    like a price/menu entry.
    """
    blob = raw.replace("\r", " ")
    # 1. primary — search with the same patterns that triggered the hit
    pats = PATTERNS.get(matched_label.lower(), [])
    for rx in pats:
        m = re.search(rx, blob, flags=re.I)
        if m:
            start = max(0, m.start() - 140)
            snippet = blob[start:start + 420]
            snippet = re.sub(r"\s+", " ", snippet).strip()
            return (snippet[:277] + "…") if len(snippet) > 280 else snippet
    # 2. secondary — bullet that contains a dollar sign or “course”
    li = re.search(r"(?:^|[\n>•-])\s*([^<>\n]{0,100}(?:\$\d{1,3}|course)[^<>\n]{0,120})",
                   blob, flags=re.I)
    if li:
        snippet = re.sub(r"\s+", " ", li.group(1)).strip()
        return (snippet[:277] + "…") if len(snippet) > 280 else snippet
    return ""
...
def ensure_schema():
    ...
    try:
        with sqlite3.connect(DB_FILE) as c:
            c.execute("SELECT deal_blurb FROM restaurants LIMIT 1")
    except sqlite3.OperationalError:  # schema mismatch
        init_db()

def store_rows(rows: List[Tuple]):
    with sqlite3.connect(DB_FILE) as c:
        c.executemany("""
        INSERT OR IGNORE INTO restaurants
        (name,address,website,has_prix_fixe,label,deal_blurb,raw_text,
...
def fetch_records(loc):
    with sqlite3.connect(DB_FILE) as c:
        return c.execute("""
        SELECT name,address,website,label,deal_blurb,snippet,review_link,
...
def process_place(place, loc):
    ...
    try:
        ...
        if matched:
            blurb   = extract_blurb(text, lbl)
            snippet = first_review(pid)
            types   = ", ".join(nice_types(g_types))
            link    = review_link(pid)
            return (name,addr,web,1,lbl,blurb,text,snippet,link,types,
                    loc,rating,photo)
    except Exception:
        pass
    return None
...
# ────────────────── UI helpers ───────────────────────────────────────────────
def build_card(name,addr,web,lbl,blurb,snippet,link,types_txt,rating,photo):
    ...
    photo_tag     = (f'<img src="https://maps.googleapis.com/maps/api/place/photo'
                     ...
                     if photo else "")
    blurb_html    = f'<p class="deal">{blurb}</p>' if blurb else ""
    snippet_html  = (f'<p class="snippet">💬 {snippet} '
                     f'<a href="{link}" target="_blank">Read&nbsp;more</a></p>'
                     if snippet else "")
    rating_html   = f'<div class="rate">{rating:.1f} / 5</div>' if rating else ""
...
.deal{font-size:.9rem;color:#222;margin:.35rem 0 .5rem;font-weight:500}
.snippet{font-size:.83rem;color:#444;margin:.25rem 0 .5rem}
...
if st.session_state.get("searched"):
    ...
    if recs:
        ...
        for g in sorted(grp.keys()):
            st.subheader(g)
            cols = st.columns(3)
            for i,(n,a,w,_,blurb,snip,lnk,ty,rating,photo) in enumerate(grp[g]):
                with cols[i%3]:
                    st.markdown(
                        build_card(n,a,w,g,blurb,snip,lnk,ty,rating,photo),
                        unsafe_allow_html=True
                    )
